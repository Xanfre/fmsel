...
#define TOGBTN_DN_CLR		fl_rgb_color(207,227,227)
...
static BOOL BackupDiffSet(FMEntry *fm)
{
	...
	if (g_cfg.bReviewDiffBackup)
	{
		...
		html.append("<br><br><b><a href=\"/cmd/accept\" color=\"#36DB8B\">");
...
public:
	...
	virtual void DoPopup()
	{
		...
		MENU_SUB($("UI Theme"));
			...
			MENU_RITEM($("Plastic"), CMD_ThemePlastic, g_cfg.uitheme == 1);
			MENU_RITEM($("Basic"), CMD_ThemeBasic, g_cfg.uitheme == 2);
			MENU_END();
...
void Fl_FM_List::draw_cell(TableContext context, int R, int C, int X, int Y, int W, int H)
{
	...
	case CONTEXT_COL_HEADER:
		...
		fl_draw_box(FL_THIN_UP_BOX, X, Y, W, H, Fl::focus()==this ? fl_rgb_color(180,180,200) : (Fl_Color)(FL_GRAY0+18));
		if (C < COL_NUM_COLS)
		{
			C = s_column_idx[C];
...
draw_sortarrow:
	...
	case CONTEXT_CELL:
		fl_push_clip(X, Y, W, H);
		{
			// bg
			if ( fm->IsInstalled() )
				fl_color(row_selected(R) ? selection_color() : ((R & 1) ? fl_rgb_color(245,245,255) : FL_WHITE));
			else if ( fm->IsAvail() )
				fl_color(row_selected(R) ? fl_rgb_color(192,239,250) : ((R & 1) ? fl_rgb_color(232,232,240) : fl_rgb_color(240,240,240)));
			else
				fl_color(row_selected(R) ? fl_rgb_color(206,238,250) : ((R & 1) ? fl_rgb_color(219,219,225) : fl_rgb_color(225,225,225)));
			fl_rectf(X, Y, W, H);
				...
				case COL_Name:
					...
					if ( !fm->IsInstalled() )
					{
						if ( fm->IsAvail() )
						{
							...
							{
								fl_color( fl_rgb_color(70,70,250) );
								if (fm->tmLastStarted)
							...
							{
								fl_color( fl_rgb_color(243,135,75) );
								fl_font(FL_HELVETICA_BOLD_ITALIC, m_fontsize);
							}
							else
							{
					...
					else
					{
						...
						{
							fl_color( fl_rgb_color(255,92,3) );
							fl_font(FL_HELVETICA_BOLD, m_fontsize);
						}
						else
						{
...
public:
	Fl_Link_Button(int X, int Y, const char *l,
		Fl_Color clr = fl_rgb_color(50,90,200), Fl_Color hoverclr = fl_rgb_color(30,50,80),
...
public:
	Fl_AddTagFilter_Button(int X, int Y, BOOL bNoFilter = FALSE)
		: Fl_Link_Button(X,Y,NULL,fl_rgb_color(50,90,200),fl_rgb_color(30,50,80),FL_HELVETICA_BOLD,FL_NORMAL_SIZE)
	{
...
protected:
	...
	static void draw_frame(int x, int y, int w, int h, Fl_Color c)
	{
		...
		fl_color( fl_rgb_color(150,185,220) );
...
public:
	...
	static const char* popup(int X, int Y, int W, int H, const char *html, BOOL (*linkcb)(Fl_Html_View*, const char*) = NULL)
	{
		...
		o->color( fl_rgb_color(60,120,180) );
...
protected:
	...
	virtual void draw()
	{
		...
		fl_color(bMouseOverX ? fl_rgb_color(145,200,245) : fl_rgb_color(130,170,200));
		fl_rectf(X, y()+1, XBUTTON_WIDTH, h()-2);
		imgTagX.draw(X+4, y()+4);

		fl_color(bFocus ? FL_BLACK : fl_rgb_color(105,135,160));
		...
		if ( !m_category.empty() )
		{
			fl_color( fl_rgb_color(95,95,100) );
...
public:
		...
		: Fl_Widget(X,Y,1,1,l)
	{
		...
		switch (op)
		{
		case FOP_AND: color( fl_rgb_color(205,220,205) ); break;
		case FOP_NOT: color( fl_rgb_color(220,205,205) ); break;
		default:
			color( fl_rgb_color(205,210,220) );
		}
		labelfont(FL_HELVETICA);
...
public:
	...
	string GenerateHtml()
	{
		...
		{
			headerstr =
			"<html><body bgcolor=\"#3C78B4\" link=\"#C8E6FA\">"
			"<table width=\"98%%\"><tr>"
				"<td width=\"20%%\"><font size=\"2.7\" color=\"#18426C\"><b>&lt;" $mid("SHIFT") "&gt; :</b> " $mid("AND filter") "</font></td>"
				"<td width=\"2%%\"></td>"
				"<td width=\"20%%\"><font size=\"2.7\" color=\"#18426C\"><b>&lt;" $mid("ALT") "&gt; :</b> " $mid("NOT filter") "</font></td>"
		...
		if (!m_bTagEdit)
		{
			if ( g_cfg.HasTagFilters() )
				_snprintf_s(buff2, sizeof(buff2), _TRUNCATE, "<a color=\"#D2C62A\" href=\"/*\">%s</a>", $("Reset Tag Filters"));
			else
				_snprintf_s(buff2, sizeof(buff2), _TRUNCATE, "<font color=\"#629FDD\">%s</font>", $("Reset Tag Filters"));
		...
		{
			if ( HasTags() )
				_snprintf_s(buff2, sizeof(buff2), _TRUNCATE, "<a color=\"#D2C62A\" href=\"/*\">%s</a>", $("Remove All Tags"));
			else
				_snprintf_s(buff2, sizeof(buff2), _TRUNCATE, "<font color=\"#629FDD\">%s</font>", $("Remove All Tags"));
		...
		for (i=0; i<(int)g_tagNameList.size();)
		{
			...
			if (!cat_is_filtered)
			{
				_snprintf_s(buff2, sizeof(buff2), _TRUNCATE, "<b><font color=\""GROUP_LABEL_CLR"\">%s:</font> <a href=\"/%s:*\">%s</a></b> (%d)",
					sCategory, buff, $tag(buff), cat_fm_count);
			}
			else
			{
				_snprintf_s(buff2, sizeof(buff2), _TRUNCATE, "<b><font color=\""GROUP_LABEL_CLR"\">%s:</font> </b><font color=\""CAT_ADDED_CLR"\"><b>%s</b> (%d)</font>",
					sCategory, $tag(buff), cat_fm_count);
			}
			_snprintf_s(buff, sizeof(buff), _TRUNCATE, table_header, buff2);
			html.append(buff);
			...
			for (j=0; j<3; j++)
			{
				...
				for (int k=0; k<ROWS && num_in_cat; i++, k++, num_in_cat--)
				{
					...
					{
						_snprintf_s(buff, sizeof(buff), _TRUNCATE, "<font color=\""TAG_ADDED_CLR"\"><b>%s</b> (%d)<font><br>",
							$tag(g_tagNameList[i]+cat_len), g_dbTagCountHash[KEY_UTF(g_tagNameList[i])]);
					}
		...
		if ( !g_tagNameListNoCat.empty() )
		{
			...
			_snprintf_s(buff2, sizeof(buff2), _TRUNCATE, "<b><font color=\""GROUP_LABEL_CLR"\">%s</font></b>", $("Misc Tags"));
			_snprintf_s(buff, sizeof(buff), _TRUNCATE, table_header, buff2);
			html.append(buff);
			...
			for (j=0; j<3; j++)
			{
				...
				for (int k=0; k<ROWS && i<(int)g_tagNameListNoCat.size(); i++, k++)
				{
					...
					{
						_snprintf_s(buff, sizeof(buff), _TRUNCATE, "<font color=\""TAG_ADDED_CLR"\"><b>%s</b> (%d)</font><br>",
							$tag(g_tagNameListNoCat[i]), g_dbTagCountHash[KEY_UTF(g_tagNameListNoCat[i])]);
					}
		...
		if (g_tagNameList.empty() && g_tagNameListNoCat.empty())
		{
			...
			"<br><br><table width=\"98%\"><tr>"
				"<td width=\"99%\" align=\"left\"><font color=\"#FFFFFF\"><b>" $mid("No tags defined") ".</b></font></td>"
				"<td width=\"1%\"></td>"
			"</tr></table><br><br>";
...
static string GenerateHtmlSummary(FMEntry *fm)
{
	...
	const char *preheader =
		"<html><body bgcolor=\"#3C78B4\" link=\""LINK_CLR"\">";

	const char *header =
		"<table width=\"98%%\"><tr bgcolor=\"#4C90D4\">"
			"<td width=\"89%%\"><font color=\""CAT_LABEL_CLR"\"><b>%s</b></font></td>"
	...
	const char *pager =
		...
		"<font face=\"courier\" size=\"2.7\" color=\"#4C90D4\"> | </font>"
	...
	_snprintf_s(buff, sizeof(buff), _TRUNCATE, pager, hasprev ? CAT_LABEL_CLR : "#4C90D4", hasnext ? CAT_LABEL_CLR : "#4C90D4");
	html.append(buff);
#endif

	_snprintf_s(buff, sizeof(buff), _TRUNCATE, header, fm->GetFriendlyName(), $("Close"));
	html.append(buff);
	...
	if (fm->tmReleaseDate)
	{
		html.append("<font color=\""CAT_LABEL_CLR"\"><u><b>");
	...
	{
		const char *compl = "<font color=\""CAT_LABEL_CLR"\"><u><b>%s:</b></u></font> %d %s%s";
		...
		_snprintf_s(buff, sizeof(buff), _TRUNCATE, compl, $("Completed"), fm->nCompleteCount,
			fm->nCompleteCount>1 ? $("times") : $("time"), buff2);
		html.append(buff);
...
static const char* GenericHtmlTextPopup(const char *caption, const char *html_body, int W, int maxH)
{
	...
	const char *header_part1 =
		"<html><body bgcolor=\"#3C78B4\" link=\""LINK_CLR"\">"
		"<br><table width=\"98%\"><tr bgcolor=\"#4C90D4\">"
		"<td width=\"89%\"><font color=\"#FFFFFF\"><b>";
	string header_part2 =
		"</b></font></td>"
...
static void ViewAbout()
{
	...
	html.append("<h1><font color=\""CAT_LABEL_CLR"\">FMSel "FMSEL_VERSION"</font></h1><br>");
	...
	sprintf(msg,
		...
		"<center><font color=\"#719DC8\"><hr></font></center>"
		...
		"</td></tr></table>"
		"<table width=\"98%%\"><tr bgcolor=\"#4C90D4\"><tr><td width=\"5%%\"></td><td width=\"95%%\">"
...
void InitProgress(int nSteps, const char *label)
{
	...
	o->color( fl_rgb_color(240,240,230) );
	o->color2( fl_rgb_color(100,170,225) );
