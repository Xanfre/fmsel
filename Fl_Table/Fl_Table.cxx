...
void Fl_Table::row_position(int row) {
  if ( _row_position == row )// OPTIMIZATION: no change? avoid redraw
  {
	  // check of top row is only partially visible, in that case do a full update
	  const int alignedtop = row_scroll_position(row);
	  if ((int)vscrollbar->Fl_Slider::value() == alignedtop)
		  return;
  }
  if ( row < 0 ) row = 0;
...
// Set a min width of a column
void Fl_Table::col_minwidth(int col, int width)
{
  if ( col < 0 ) return;
  if ( col < (int)_colminwidths.size() && _colminwidths[col] == width ) {
    return;			// OPTIMIZATION: no change? avoid redraw
  }
  // Add column widths, even if none yet
  int now_size = (int)_colminwidths.size();
  if ( col >= now_size ) {
    _colminwidths.size(col+1);
    while (now_size < col) {
      _colminwidths[now_size++] = width;
    }
  }
  _colminwidths[col] = width;
}
...
void Fl_Table::scroll_cb(Fl_Widget*, void *data) {
...
int Fl_Table::move_cursor(int R, int C, bool bIgnoreShift) {
  if (select_row == -1) R++;
  if (select_col == -1) C++;
  R += select_row;
  C += select_col;
  ...
  select_row = R;
  select_col = C;
  if (bIgnoreShift || !Fl::event_state(FL_SHIFT)) {
    current_row = R;
    current_col = C;
  }
  //if (R < toprow + 1 || R > botrow - 1) row_position(R);
  if (R < toprow + 1 || R > botrow - 1) ensure_visible(R);
...
int Fl_Table::handle(int event) {
  ...
  switch ( event ) {
	...
	case CONTEXT_COL_HEADER:
	  ...
	  if ( Fl::event_button() == FL_LEFT_MOUSE) {
	    ...
	    if ( resizeflag ) {
		  ...
		  if (col_minwidth(_resizing_col) >= 0)
		  {
			  _resizing_row = -1;
	...
	int min_w = col_minwidth(_resizing_col);
    ...
    case FL_RELEASE:
      _stop_auto_drag();
	  if (_resizing_col > -1 || _resizing_row > -1)
	  {
		  if ( Fl_Widget::callback() && (when() & (FL_WHEN_RELEASE|FL_WHEN_CHANGED)) ) {
			  do_callback(CONTEXT_RC_RESIZED, R, C);
		  }
	  }
      switch ( context ) {
...
//	       _last_row == R ) {		// release on same row PUSHed?
	       (_last_row == R || context == CONTEXT_CELL) ) {		// release on same row PUSHed?
    ...
    case FL_MOVE:
      if ( context == CONTEXT_COL_HEADER && 		// in column header?
	   resizeflag && col_minwidth((resizeflag&RESIZE_COL_LEFT)?C-1:C) >= 0) {// resize + near boundary?
...
void Fl_Table::ensure_visible(int R, int bCentered)
{
	if (bCentered && R > 0 && R < rows()-1)
	{
		// make sure row is in the middle of list
		...
		if (bCentered == 2)
		{
			...
			if (R >= toprow && R <= botrow)
			{
				if (R == toprow)
				{
					const int alignedtop = row_scroll_position(R);
					if ((int)vscrollbar->Fl_Slider::value() == alignedtop)
						return;
				}
				else if (R == botrow)
				{
					const int alignedbottom = row_scroll_position(R) + row_height(R);
					if ((int)vscrollbar->Fl_Slider::value() + tih >= alignedbottom)
						return;
				}
				else
					return;
		...
		int H = (tih - row_height(R)) / 2;
		while (H > 0 && R > 0)
		{
			const int rh = row_height(R-1);
			if (rh > H) break;
			H -= rh;
			R--;
		}
	...
	{
		// calc the top row needed for R to be entirely visible at the bottom of the list
		int H = tih - row_height(R);
		while (H > 0 && R > 0)
		{
			const int rh = row_height(R-1);
			if (rh > H) break;
