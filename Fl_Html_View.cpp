//
// "$Id: Fl_Html_View.cxx 6745 2009-04-07 18:37:25Z AlbrechtS $"
//
// Fl_Html_View widget routines.
// (basically a copy of Fl_Help_View with some minor tweaks)
//
...
// Contents:
//
//   Fl_Html_View::add_block()       - Add a text block to the list.
//   Fl_Html_View::add_link()        - Add a new link to the list.
//   Fl_Html_View::add_target()      - Add a new target to the list.
//   Fl_Html_View::compare_targets() - Compare two targets.
//   Fl_Html_View::do_align()        - Compute the alignment for a line in
//                                     a block.
//   Fl_Html_View::draw()            - Draw the Fl_Html_View widget.
//   Fl_Html_View::format()          - Format the help text.
//   Fl_Html_View::format_table()    - Format a table...
//   Fl_Html_View::free_data()       - Free memory used for the document.
//   Fl_Html_View::get_align()       - Get an alignment attribute.
//   Fl_Html_View::get_attr()        - Get an attribute value from the string.
//   Fl_Html_View::get_color()       - Get an alignment attribute.
//   Fl_Html_View::handle()          - Handle events in the widget.
//   Fl_Html_View::Fl_Html_View()    - Build a Fl_Html_View widget.
//   Fl_Html_View::~Fl_Html_View()   - Destroy a Fl_Html_View widget.
//   Fl_Html_View::load()            - Load the specified file.
//   Fl_Html_View::resize()          - Resize the help widget.
//   Fl_Html_View::topline()         - Set the top line to the named target.
//   Fl_Html_View::topline()         - Set the top line by number.
//   Fl_Html_View::value()           - Set the help text directly.
...
//
// Simple margin stack for Fl_Html_View::format()...
//
...
//
// All the stuff needed to implement text selection in Fl_Html_View
//
 ...
 * only to not enlarge the Fl_Html_View class. Lucky for us, only one
...
static Fl_Offscreen fl_html_view_buffer;
int Fl_Html_View::selection_first = 0;
int Fl_Html_View::selection_last = 0;
int Fl_Html_View::selection_push_first = 0;
int Fl_Html_View::selection_push_last = 0;
int Fl_Html_View::selection_drag_first = 0;
int Fl_Html_View::selection_drag_last = 0;
int Fl_Html_View::selected = 0;
int Fl_Html_View::draw_mode = 0;
int Fl_Html_View::mouse_x = 0;
int Fl_Html_View::mouse_y = 0;
int Fl_Html_View::current_pos = 0;
Fl_Html_View *Fl_Html_View::current_view = 0L;
Fl_Color Fl_Html_View::hv_selection_color;
Fl_Color Fl_Html_View::hv_selection_text_color;
 ...
 */
void Fl_Html_View::hv_draw(const char *t, int x, int y)
{
...
Fl_Html_Block *					// O - Pointer to new block
Fl_Html_View::add_block(const char   *s,	// I - Pointer to start of block text
...
//         s, xx, yy, ww, hh, border);
  ...
  if (nblocks_ >= ablocks_)
  {
    ...
    if (ablocks_ == 16)
      blocks_ = (Fl_Html_Block *)malloc(sizeof(Fl_Html_Block) * ablocks_);
    else
      blocks_ = (Fl_Html_Block *)realloc(blocks_, sizeof(Fl_Html_Block) * ablocks_);
  }

  temp = blocks_ + nblocks_;
  memset(temp, 0, sizeof(Fl_Html_Block));
  temp->start   = s;
  temp->end     = s;
  temp->x       = xx;
  temp->y       = yy;
		      ...
		      int        hh)	// I - Height of link text
{
  ...
  if (nlinks_ >= alinks_)
  {
    ...
    if (alinks_ == 16)
      links_ = (Fl_Html_Link *)malloc(sizeof(Fl_Html_Link) * alinks_);
    else
      links_ = (Fl_Html_Link *)realloc(links_, sizeof(Fl_Html_Link) * alinks_);
  }
...
void
Fl_Html_View::add_target(const char *n,	// I - Name of target
                	int        yy)	// I - Y position of target
{
  Fl_Html_Target	*temp;			// New target
  ...
  if (ntargets_ >= atargets_)
  {
    ...
    if (atargets_ == 16)
      targets_ = (Fl_Html_Target *)malloc(sizeof(Fl_Html_Target) * atargets_);
    else
      targets_ = (Fl_Html_Target *)realloc(targets_, sizeof(Fl_Html_Target) * atargets_);
  }
...
Fl_Html_View::compare_targets(const Fl_Html_Target *t0,	// I - First target
                             const Fl_Html_Target *t1)	// I - Second target
{
...
//
// 'Fl_Html_View::do_align()' - Compute the alignment for a line in a block.
//

int						// O - New line
Fl_Html_View::do_align(Fl_Html_Block *block,	// I - Block to add to
...
{
	int			i;		// Looping var
	const Fl_Html_Block	*block;		// Pointer to current block
					...
					*s = '\0';
					...
					else if (strcasecmp(buf, "BR") == 0)
					{
						if (line < 31)
							line ++;
						xx = block->line[line];
						if (!hh) hh = fsize + 2;// Fl_Html_View mod: make <br> tags work for empty lines
						yy += hh;
						hh = 0;
					}
...
//							buf[fl_unicode2utf(b, 1, buf)] = 0;
					...
					{
						// Fl_Html_View mod: allow to customize color per link
						if (get_attr(attrs, "COLOR", attr, sizeof(attr)) != NULL) {
							fl_color(get_color(attr, linkcolor_));
						}
						else
...
//
// 'Fl_Html_View::find()' - Find the specified string...
//

int						// O - Matching position or -1 if not found
Fl_Html_View::find(const char *s,		// I - String to find
                   int        p)		// I - Starting position
{
  int		i,				// Looping var
		c;				// Current character
  Fl_Html_Block	*b;				// Current block
...
Fl_Html_View::format()
{
	...
	Fl_Html_Block	*block,		// Current block
				...
				*s = '\0';
					...
					strcasecmp(buf, "TABLE") == 0)
				{
					...
					else if (strcasecmp(buf, "TABLE") == 0)
					{
						...
						// Fl_Html_View mod: disable initial_load for table format, or image ref counts will be wrong
						int org_initial_load = initial_load;
						initial_load = 0;

						format_table(&table_width, columns, start);

						initial_load = org_initial_load;

						// Fl_Html_View mod: not sure if it breaks something but it fixes my needs
						block->w = table_width;
						force_block_width = 1;
				...
				else if (strcasecmp(buf, "TR") == 0)
				{
					...
					hh        = 0;
					// Fl_Html_View mod: use table width, not sure if it breaks something but it fixes my needs
					//block     = add_block(start, xx, yy, hsize_, 0);
					block     = add_block(start, xx, yy, table_width, 0);
					row       = block - blocks_;
					needspace = 0;
					column    = 0;
					line      = 0;
					...
					strcasecmp(buf, "TH") == 0) && row)
				{
					...
					if ( get_attr(attrs, "HEIGHT", attr, sizeof(attr)) )
						block->h = get_length(attr);
				}
				else if ((strcasecmp(buf, "/TD") == 0 ||
					strcasecmp(buf, "/TH") == 0) && row)
				{
	...
	if (ntargets_ > 1)
		qsort(targets_, ntargets_, sizeof(Fl_Html_Target),
...
void
Fl_Html_View::format_table(int        *table_width,	// O - Total table width
	                   ...
	                   const char *table)		// I - Pointer to start of table
{
	...
	// Fl_Html_View mod: calc max table size so %-based cell widths work correctly (this will be as broken as
	// the original code for auto-sized tables, but we only need for tables where a table width is specified)
	int max_table_width = hsize_ - Fl::scrollbar_size();
	if (get_attr(table + 6, "WIDTH", attr, sizeof(attr)))
		max_table_width = get_length(attr);
	...
	*table_width = 0;
			...
			*s = '\0';
				...
				strcasecmp(buf, "TH") == 0)
			{
				...
				if (get_attr(attrs, "WIDTH", attr, sizeof(attr)) != NULL)
					// Fl_Html_View mod: fix for % based width of table cells
					//max_width = get_length(attr);
					max_width = get_length(attr, max_table_width);
				else
					max_width = 0;
	...
	// Add up the widths...
	for (column = 0, width = -6; column < num_columns; column ++)
		width += columns[column] + 6;
...
//
// 'Fl_Html_View::is_shared_image()' - Check if image was loaded from disk and not a native resource
//

bool Fl_Html_View::is_shared_image(Fl_Image *img) const
{
	if ((void*)img != &broken_image)
	{
		// search shared images
		Fl_Shared_Image **shared = Fl_Shared_Image::images();
...
int					// O - Alignment
Fl_Html_View::get_align(const char *p,	// I - Pointer to start of attrs
...
Fl_Html_View::get_attr(const char *p,		// I - Pointer to start of attributes
...
//
// 'Fl_Html_View::get_color()' - Get a color attribute.
//

Fl_Color				// O - Color value
Fl_Html_View::get_color(const char *n,	// I - Color name
...
//
// 'Fl_Html_View::get_image()' - Get an inline image.
//
...
/** Gets an inline image.
  ...
  \todo Fl_Html_View::get_image() returns a pointer to the internal
...
/* Implementation note: (A.S. Apr 05, 2009)
  ...
  Fl_Html_View::get_image() uses a static global flag (initial_load)
  ...
  structure like Fl_Html_Target (Fl_Help_Image ?) to avoid using this
...
Fl_Image *
Fl_Html_View::get_image(const char *name, int W, int H) {
  ...
  if ( !strncmp(name, "imgres:", 7) )
  {
	  if (imgcb_)
	  {
		Fl_Image *im = (*imgcb_)(name + 7);
		if (im)
			return im;
	  }
...
//
// 'Fl_Html_View::get_length()' - Get a length value, either absolute or %.
//

int
Fl_Html_View::get_length(const char *l) {	// I - Value
...
// Fl_Html_View mod: get length using 'width' as reference (used for relative width of table cells)
int Fl_Html_View::get_length(const char *l, int width)
{
  int	val;					// Integer value
...
Fl_Html_Link *Fl_Html_View::find_link(int xx, int yy)
{
  int		i;
  Fl_Html_Link	*linkp;
...
void Fl_Html_View::follow_link(Fl_Html_Link *linkp)
{
...
void Fl_Html_View::clear_global_selection()
{
  if (selected) redraw();
...
char Fl_Html_View::begin_selection()
{
  clear_global_selection();

  if (!fl_html_view_buffer) fl_html_view_buffer = fl_create_offscreen(1, 1);
  ...
  draw_mode = 1;
    ...
    current_view = this;
    fl_begin_offscreen(fl_html_view_buffer);
    draw();
    fl_end_offscreen();
...
char Fl_Html_View::extend_selection()
{
...
void Fl_Html_View::end_selection(int clipboard) 
{
...
Fl_Html_View::handle(int event)	// I - Event to handle
{
  static Fl_Html_Link *linkp;   // currently clicked link
...
Fl_Html_View::Fl_Html_View(int        xx,	// I - Left position
                  ...
                  ww - Fl::scrollbar_size(), Fl::scrollbar_size())
{
  ...
  ablocks_      = 0;
  nblocks_      = 0;
  blocks_       = (Fl_Html_Block *)0;

  nfonts_       = 0;

  link_         = (Fl_Html_Func *)0;

  alinks_       = 0;
  nlinks_       = 0;
  links_        = (Fl_Html_Link *)0;

  atargets_     = 0;
  ntargets_     = 0;
  targets_      = (Fl_Html_Target *)0;
...
Fl_Html_View::load(const char *f)// I - Filename to load (may also have target)
{
...
void
Fl_Html_View::resize(int xx,	// I - New left position
...
void
Fl_Html_View::topline(const char *n)	// I - Target name
{
  Fl_Html_Target key,			// Target name key
  ...
  target = (Fl_Html_Target *)bsearch(&key, targets_, ntargets_, sizeof(Fl_Html_Target),
                                 (compare_func_t)compare_targets);
...
void
Fl_Html_View::topline(int t)	// I - Top line number
{
  if (!value_)
    return;
  ...
  // Fl_Html_View mod: only subtract horizontal scrollbar height if it's visible
  //scrollbar_.value(topline_, h() - Fl::scrollbar_size(), 0, size_);
  scrollbar_.value(topline_, h() - (hscrollbar_.visible() ? Fl::scrollbar_size() : 0), 0, size_);
...
//
// 'Fl_Html_View::leftline()' - Set the left position.
//

void
Fl_Html_View::leftline(int l)	// I - Left position
{
  if (!value_)
    return;
...
void
Fl_Html_View::value(const char *v)	// I - Text to view
{
  clear_selection();
  free_data();
  set_changed();
...
//
// End of "$Id: Fl_Html_View.cxx 6745 2009-04-07 18:37:25Z AlbrechtS $".
//
